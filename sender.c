...
float batteryTemperature [SENSOR_DATA_SIZE] = {16.0,22.0,6.0,24.0,28.0,19.0,43.0,36.0,50.0,30.0,
                                               45.0,42.0,50.0,48.0,39.0,38.0,49.0,1.0,40.0,5.0,
...
float batteryChargingCurrent [SENSOR_DATA_SIZE] = {2.6,1.6,3.6,4.6,5.6,6.6,7.6,8.6,9.6,3.1,
                                                   6.7,6.9,7.0,7.2,8.4,8.6,5.8,4.5,5.6,9.9,
...
int main ()
{
	...
	for (int readIndex = 0; readIndex < SENSOR_DATA_SIZE; readIndex ++)
	{
		sensor_data.temperature = batteryTemperature[readIndex];
		sensor_data.chargingCurrent = batteryChargingCurrent[readIndex];
		postProcessingSensorData (sensor_data, sensorDataStream);
		streamData(sensorDataStream, endPoint);
	}
